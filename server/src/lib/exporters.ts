type ShiftCode = "M" | "A" | "N" | "X" | "H" | "AL" | "ML" | "SL";
type LeaveCode = "AL" | "ML" | "SL";
type RosterGrid = Record<string, Record<string, string>>;
type Staff = { id: string; name: string; rank: string };
type DayInfo = { day: number; iso: string; dowLabel: string };
const MONTHS = ["January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"];
const isLeaveCode = (code: string): code is LeaveCode => code === "AL" || code === "ML" || code === "SL";

export function buildExportDays(startDate: string, endDate: string): DayInfo[] {
  const result: DayInfo[] = [];
  const cursor = new Date(`${startDate}T00:00:00Z`);
  const end = new Date(`${endDate}T00:00:00Z`);
  while (cursor <= end) {
    result.push({ day: cursor.getUTCDate(), iso: cursor.toISOString().slice(0, 10), dowLabel: ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"][cursor.getUTCDay()] });
